...
<c:choose>
	<c:when test="<%= !group.isLayoutPrototype() %>">
		<c:if test="<%= !layoutsAdminDisplayContext.isDraft() && !selLayout.isSystem() %>">
			...
			<aui:input aria-describedby='<%= liferayPortletResponse.getNamespace() + "customizableDescription" %>' label="show-page-in-menu-display" labelCssClass="font-weight-normal" name="hidden" type="checkbox" value="<%= !selLayout.isHidden() %>" wrapperCssClass="mb-2" />
